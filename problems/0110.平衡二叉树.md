...
### 自上而下（先序）
1.求出当前节点的左右子树的高度，判断以当前节点的子树是否平衡
2.递归判断该节点的左右子树是否平衡
```java
class Solution {
    public boolean isBalanced(TreeNode root) {
        ...
        int leftHeiht = height(root.left);
        int rightHeight = height(root.right);
        
        return Math.abs(leftHeiht - rightHeight) <= 1 && isBalanced(root.left) && isBalanced(root.right);
    }

    public int height(TreeNode node){
        if(node == null) return 0;

        int leftHeiht = height(node.left);
        int rightHeight = height(node.right);

        return Math.max(leftHeiht, rightHeight) + 1;
    }
}
```
...
**-1：当前子树是不平衡的或者当前子树包含不平衡的子树，当某个节点的子树中出现-1，由递归式可以立即返回-1，不走求高度**
...
代码如下：
```java
if(leftHeiht == -1 || rightHeight == -1 || Math.abs(leftHeiht - rightHeight) > 1){
            return -1;
```
