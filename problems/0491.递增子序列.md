...
class Solution {
    ...
    void backtracking(int[] nums, int startIndex){
        ...
        // 由于不能排序，所以不能用之前的逻辑
        // 对每个父节点建立一个子节点的使用情况数组
        Map<Integer, Boolean> map = new HashMap<>();
        for(int i = startIndex;  i< nums.length; i++){
            // 递增子序列，不要求连续，所以对当子序列尾，可以在该层不断找可以构成递增子序列的元素
            if(map.getOrDefault(nums[i], false) == true || (!path.isEmpty() && nums[i] < path.get(path.size() - 1))) continue;  

            path.offerLast(nums[i]);
            map.put(nums[i], true);

            backtracking(nums, i + 1);
		   // 由于判断父节点的子节点是否之前遍历过，所以这里不需要设为false
            path.pollLast();
