...
边统计path，边计算该path的路径权值和。

由于在回溯和递归中，path都会根据节点情况随时改变，可以定义为全局变量。
...
for (int i = startIndex; i <= 9; i++) {
    sum += i;
    path.offerLast(i);
    backtracking(k, sum, i + 1); // 注意i+1调整startIndex
    sum -= i; // 回溯
    path.pollLast(); // 回溯
}
```
...
参照[关于回溯算法，你该了解这些！](https://programmercarl.com/回溯算法理论基础.html)中的模板，不难写出如下JAVA代码：
...
和[回溯算法：组合问题再剪剪枝](https://programmercarl.com/0077.组合优化.html) 一样，for循环的范围也可以剪枝，i <= 9 - (k - path.size()) + 1, 再往右边走，个数不够3个。
